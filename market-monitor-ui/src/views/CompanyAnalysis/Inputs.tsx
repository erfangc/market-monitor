...
interface Props {
    ticker?: string
    onChange: (request: CompanyAnalysisRequest) => void
}

const now = moment();
const date = now.format("YYYY-MM-DD");

const initialState: CompanyAnalysisRequest = {
    date,
    longTermGrowth: 0.0,
    shortTermEpsGrowths: [
        {
...
export function Inputs(props: Props) {
    ...
    const [request, setRequest] = useState<CompanyAnalysisRequest>(initialState);
    const [tickerOptions, setTickerOptions] = useState<Asset[]>([]);
    ...
    async function autoPopulateRequest(ticker?: string) {
        const apiResponse = await axios
            .get<CompanyAnalysisRequest>(
                `/api/company-analysis/request/${ticker}`
            );
    ...
    function handleTickerSelect(ticker: string) {
        history.push(`/company-analysis/${ticker}`);
    }

    async function handleTickerSearch(term: string) {
        const {data} = await axios.get<Asset[]>('/api/assets/_match', {params: {term}});
